...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (!mongoConnected) {
      console.log('🔧 Using temporary admin fallback - database not connected');
      if (email.toLowerCase() === 'admin@campus.edu' && password === 'CampusAdmin2024!' && role === 'admin') {
        const token = jwt.sign(
          { id: 'admin-temp', email: email.toLowerCase(), role: 'admin', name: 'System Administrator' },
          JWT_SECRET,
          { expiresIn: '24h' }
        );
        ...
        return res.status(503).json({ error: 'Database connection required for user registration. Please connect to Neon database or contact administrator.' });
      }
    }
...
app.get('/api/organizers', authenticateToken, requireRole(['admin']), async (req, res) => {
  try {
    if (!mongoConnected) {
      console.log('🔧 Returning empty organizers - database not connected');
      return res.json([]);
    }
...
app.get('/api/events', authenticateToken, async (req, res) => {
  try {
    if (!mongoConnected) {
      console.log('🔧 Returning empty events - database not connected');
      return res.json([]);
    }
